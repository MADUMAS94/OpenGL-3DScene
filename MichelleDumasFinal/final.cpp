...
namespace
{
const char* const WINDOW_TITLE = "Michelle Dumas: Final 3D scene"; // Macro for window title
...
const GLchar* fragmentShaderSource = GLSL(440,
    ...
    uniform float ambientStrength = 0.2f; // Set ambient or global lighting strength
    uniform float specularIntensity1 = 0.1f;
    uniform float highlightSize1 = 0.1f;
    uniform float specularIntensity2 = 0.1f;
    uniform float highlightSize2 = 0.1f;
    uniform float specularIntensity3 = 0.1f;
    uniform float highlightSize3 = 0.1f;
    uniform float specularIntensity4 = 0.1f;
    uniform float highlightSize4 = 0.1f;
    ...
    void main()
    {
        ...
        phong2 = (ambient + diffuse2 ) * textureColor.xyz;
        phong3 = (ambient + diffuse3 ) * textureColor.xyz;
        phong4 = (ambient + diffuse4 ) * textureColor.xyz;
...
int main(int argc, char* argv[])
{
    ...
    // Load texture
    const char* texFilename = "resources/textures/coke.png";
    const char* texFilename1 = "resources/textures/Desk.png";
    const char* texFilename2 = "resources/textures/Cap.png";
    const char* texFilename3 = "resources/textures/Bottle.png";
    const char* texFilename4 = "resources/textures/Orange.png";
    const char* texFilename5 = "resources/textures/plastic.png";
    const char* texFilename6 = "resources/textures/metal.png";
    const char* texFilename7 = "resources/textures/cantop.png";
    const char* texFilename8 = "resources/textures/eraser.png";
    const char* texFilename9 = "resources/textures/lead.png";
    if (!UCreateTexture(texFilename, coke))
    {
        cout << "Failed to load texture " << texFilename << endl;
        return EXIT_FAILURE;
    }
    if (!UCreateTexture(texFilename1, desk))
    {
        cout << "Failed to load texture " << texFilename1 << endl;
        return EXIT_FAILURE;
    }
    if (!UCreateTexture(texFilename2, cap))
    {
        cout << "Failed to load texture " << texFilename2 << endl;
        return EXIT_FAILURE;
    }
    if (!UCreateTexture(texFilename3, bottle))
    {
        cout << "Failed to load texture " << texFilename3 << endl;
        return EXIT_FAILURE;
    }
    if (!UCreateTexture(texFilename4, orange))
    {
        cout << "Failed to load texture " << texFilename4 << endl;
        return EXIT_FAILURE;
    }
    if (!UCreateTexture(texFilename5, plastic))
    {
        cout << "Failed to load texture " << texFilename5 << endl;
        return EXIT_FAILURE;
    }
    if (!UCreateTexture(texFilename6, metal))
    {
        cout << "Failed to load texture " << texFilename6 << endl;
        return EXIT_FAILURE;
    }
    if (!UCreateTexture(texFilename7, cantop))
    {
        cout << "Failed to load texture " << texFilename7 << endl;
        return EXIT_FAILURE;
    }
    if (!UCreateTexture(texFilename8, eraser))
    {
        cout << "Failed to load texture " << texFilename6 << endl;
        return EXIT_FAILURE;
    }
    if (!UCreateTexture(texFilename9, lead))
    {
        cout << "Failed to load texture " << texFilename7 << endl;
        return EXIT_FAILURE;
    }
    ...
    // Release texture
    UDestroyTexture(coke);
    UDestroyTexture(desk);
    UDestroyTexture(cap);
    UDestroyTexture(bottle);
    UDestroyTexture(orange);
    UDestroyTexture(plastic);
    UDestroyTexture(metal);
    UDestroyTexture(cantop);
    UDestroyTexture(eraser);
    UDestroyTexture(lead);
...
void URender()
{
    ...
    //Loop to switch from perspective to ortho view
    glm::mat4 projection;
    if (viewProjection) {
        projection = glm::perspective(glm::radians(gCamera.Zoom), (GLfloat)WINDOW_WIDTH / (GLfloat)WINDOW_HEIGHT, 0.1f, 100.0f);
    }
        ...
        float scale = 100;
        projection = glm::ortho((1200.0f / scale), -(1200.0f / scale), -(800.0f / scale), (800.0f / scale), 0.1f, 100.0f);
    }
    ...
    glUniform3f(light1PosLoc, 1.0f, 6.0f, 10.0f);

    glUniform3f(light2ColLoc, 0.5f, 0.5f, 0.5f);
    glUniform3f(light2PosLoc, 12.0f, 3.0f, 1.0f);

    glUniform3f(light3ColLoc, 0.5f, 0.5f, 0.5f);
    glUniform3f(light3PosLoc, -8.0f, 5.0f, -2.0f);

    glUniform3f(light4ColLoc, 0.5f, 0.5f, 0.5f);
    glUniform3f(light4PosLoc, 2.0f, 3.0f, -10.0f);

    glUniform1f(specInt1Loc, 0.7f);
    glUniform1f(specInt2Loc, .1f);
    glUniform1f(specInt3Loc, .1f);
    glUniform1f(specInt4Loc, .1f);

    glUniform1f(highlghtSz1Loc, 5.0f);
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, bottle);

    // 1. Scales the object
    scale = glm::scale(glm::vec3(1.0f, 2.5f, 1.0f));
    // 2. Rotate the object
    rotation = glm::rotate(60.0f, glm::vec3(0.0f, 1.0f, 0.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(5.0f, 1.0f, 1.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.7f, .6f, 0.7));
    // 2. Rotate the object
    rotation = glm::rotate(0.0f, glm::vec3(1.0f, 1.0f, 1.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(5.0f, 3.35f, 1.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.90f, 1.0f, 0.90));
    // 2. Rotate the object
    rotation = glm::rotate(0.0f, glm::vec3(1.0f, 1.0f, 1.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(5.0f, 2.7f, 1.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    rotation = glm::rotate(0.0f, glm::vec3(1.0f, 1.0f, 1.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(5.0f, 3.9f, 1.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.905f, 0.905f, 3.0f));
    // 2. Rotate the object
    rotation = glm::rotate(90.0f, glm::vec3(-5.5, -5.0f, -5.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(5.0f, 3.42f, 1.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, coke);

    // 1. Scales the object
    scale = glm::scale(glm::vec3(1.8f, 5.0f, 1.8f));
    // 2. Rotate the object
    rotation = glm::rotate(0.0f, glm::vec3(1.0f, 1.0f, 2.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(2.5f, 1.5f, -2.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 2. Rotate the object
    rotation = glm::rotate(3.142f, glm::vec3(1.0, 0.0f, 0.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(2.5f, 1.5f, -2.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(1.3f, 1.3f, 4.0f));
    // 2. Rotate the object
    rotation = glm::rotate(90.0f, glm::vec3(-5.5, -5.0f, -5.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(2.5f, 1.4f, -2.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(1.8f, 0.4f, 1.8f));
    // 2. Rotate the object
    rotation = glm::rotate(0.0f, glm::vec3(1.0, 0.0f, 0.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(2.5f, 6.5f, -2.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(1.45f, 1.45f, 4.0f));
    // 2. Rotate the object
    rotation = glm::rotate(90.0f, glm::vec3(-5.5, -5.0f, -5.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(2.5f, 6.6f, -2.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(1.5f, 0.4f, 1.5f));
    // 2. Rotate the object
    rotation = glm::rotate(0.0f, glm::vec3(1.0f, 1.0f, 1.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(2.5f, 6.51f, -2.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.2f, 7.0f, 0.2f));
    // 2. Rotate the object
    rotation = glm::rotate(4.61f, glm::vec3(1.0f, 1.0f, -3.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(7.0f, 1.25f, 6.0f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.2f, 0.9f, 0.2f));
    // 2. Rotate the object
    rotation = glm::rotate(4.61f, glm::vec3(1.0f, 1.0f, -3.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(1.4f, 1.24f, 1.8f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.04f, 1.1f, 0.04f));
    // 2. Rotate the object
    rotation = glm::rotate(4.61f, glm::vec3(1.0f, 1.0f, -3.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(1.4f, 1.24f, 1.8f));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.13f, 2.0f, 0.13f));
    // 2. Rotate the object
    rotation = glm::rotate(4.61f, glm::vec3(1.0f, 1.0f, -3.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(7.5f, 1.24f, 6.37f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.2f, 0.3f, 0.2f));
    // 2. Rotate the object
    rotation = glm::rotate(4.61f, glm::vec3(1.0f, 1.0f, -3.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(7.4f, 1.24f, 6.3f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, eraser);

    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.14f, 0.4f, 0.14f));
    // 2. Rotate the object
    rotation = glm::rotate(4.61f, glm::vec3(1.0f, 1.0f, -3.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(7.5f, 1.24f, 6.37f));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.2f, 0.3f, 0.3f));
    // 2. Rotate the object
    rotation = glm::rotate(4.61f, glm::vec3(1.0f, 1.0f, -3.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(6.0f, 1.24f, 5.5f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
    ...
    // 1. Scales the object
    scale = glm::scale(glm::vec3(0.2f, 1.2f, 0.02f));
    // 2. Rotate the object
    rotation = glm::rotate(4.61f, glm::vec3(1.0f, 1.0f, -3.0f));
    // 3. Position the object
    translation = glm::translate(glm::vec3(5.5f, 1.24f, 5.3f));
    // Model matrix: transformations are applied right-to-left order
    model = translation * rotation * scale;
    glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
